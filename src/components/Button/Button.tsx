...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  ...
  const iconClass = useMemo(() => (size === 'small' ? 'text-lg' : 'text-2xl'), [
    size,
  ]);

  const variantClass = useMemo(() => {
    if (variant === 'contained')
      return `text-white ${ButtonVariant[variant]}-${buttonColorClass}`;

    if (variant === 'outlined')
      return `border-2 text-${buttonColorClass} ${ButtonVariant[variant]}-${buttonColorClass}`;
  ...
  return (
    ...
    >
      {prefix}
      {children && (
        <span className={`${size ? ButtonSize[size] : ''}`}>{children}</span>
      )}
      {suffix}
    </button>
  );
